...
public abstract class ConjureFluidComponent extends SpellEffect {
    ...
    public ConjureFluidComponent(final ResourceLocation guiIcon, Fluid fluid, AttributeValuePair... attributeValuePairs) {
    ...
    public boolean tryPlaceFluid(Player player, Level world, BlockPos blockPos, IModifiedSpellPart<SpellEffect> modificationData, boolean ignoreVaporize) {
        ...
        if (world.dimensionType().ultraWarm() && resource.getFluid().getFluidType().isVaporizedOnPlacement(world, blockPos, resource) && !(ignoreVaporize && CommonConfig.CAN_CONJURE_FLUID_IGNORE_VAPORIZE.get())) {
