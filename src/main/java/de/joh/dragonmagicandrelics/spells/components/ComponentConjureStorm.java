...
public class ComponentConjureStorm extends SpellEffect {
    ...
    public ComponentApplicationResult ApplyEffect(SpellSource source, SpellTarget target, IModifiedSpellPart<SpellEffect> modificationData, SpellContext context) {
        if (1.5F <= (modificationData.getValue(Attribute.MAGNITUDE))) {
            ((ServerLevel)context.getWorld()).setWeatherParameters(0, 6000, true, true);
        } else {
            ((ServerLevel)context.getWorld()).setWeatherParameters(0, 6000, true, false);
        }
